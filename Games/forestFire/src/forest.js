...
import {
  ...
  final_conversation,
  realDialogueData,
  pre_ice_level_conversation,
  pre_parking_scene_conversation,
  post_ice_level_conversation,
  pre_cat_level_conversation,
  post_cat_level_conversation,
  pre_forest_level_conversation,
  pre_desert_level_conversation,
  post_desert_level_conversation,
} from "./constants";
...
k.scene("forest", async ({ hasConversation }) => {
  ...
  const mapData = await (
    await fetch("./assets/game/maze-of-trees.json")
  ).json();
  const layers = mapData.layers;

  const map = k.add([k.sprite("forest"), k.pos(0), k.scale(scaleFactor / 2)]);
        ...
        if (boundary.name) {
          player.onCollide(boundary.name, () => {
            if (boundary.name === "final") {
              window.location.href = "http://localhost:3000/learn-play";
            } else {
              player.isInDialogue = true;
...
k.scene("lose", async ({ backTo }) => {
  ...
  k.add([
    k.text(
      "Looks like the Climate Crisis' efects are not so easy to ignore, huh?"
    ),
    k.pos(k.width() / 2, k.height() / 2 + 58),
    k.scale(0.75),
    k.anchor("center"),
...
k.go("transition", {
  conversation: post_cat_level_conversation,
  nextScene: "forest",
});

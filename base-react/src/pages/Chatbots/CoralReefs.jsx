...
const CoralReefChat = () => {
  ...
  const handleSend = async () => {
    ...
    const prompt = `
      You're a friendly and wise Coral Reef, guiding users about marine life and ocean conservation.
    ...
    try {
      ...
      const aiResponseText = result?.response?.candidates?.[0]?.content?.parts?.[0]?.text ?? 
        "I'm not sure how to answer that, but let's explore the ocean together!";

      setMessages((prev) => [...prev, { sender: "coralReef", text: aiResponseText }]);
    } catch (error) {
      console.error("Error fetching AI response:", error);
      setMessages((prev) => [...prev, { sender: "coralReef", text: "Oops! Something went wrong. Try again later." }]);
    }
  ...
  return (
    ...
    //   style={{ backgroundImage: "url('https://images.pexels.com/photos/355775/pexels-photo-355775.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260')" }}
    >
      <div className="text-center mb-6">
        <div className="text-2xl font-bold text-blue-900 flex items-center justify-center gap-2 animate-bounce">
          <span className="text-4xl">🪸</span>
          Chat with the Coral Reef!
          <span className="text-4xl"></span>
        </div>
      </div>
            ...
            <div key={index} className={`flex ${message.sender === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`${message.sender === "user" ? "bg-green-100" : "bg-blue-200"} rounded-2xl p-3 max-w-[75%] shadow-sm`}>
                <div className="flex items-center gap-2">
                  {message.sender === "coralReef" && <span className="text-2xl">🪸</span>}
      ...
      <div className="text-center text-blue-900 text-sm">
        <p>🌊 Let's protect our oceans together! 🌊</p>
      </div>
    </div>
  );
};

export default CoralReefChat;
